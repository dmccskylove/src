...
{
	if( c_pgvVisionManager->m_isNeed_UpdateInterface )
    {
        c_pgvVisionManager->m_isNeed_UpdateInterface = false;
...
void ISBOTCAPMain::OnLeftDown( wxMouseEvent& event )
{
    ...
    int idxSelected = c_pgvVisionManager->get_idxActiveCCD();
	gvVisionCCD* pgvVisionCCD = c_pgvVisionManager->gvMgr_getVisionCCD(idxSelected);
	gvVisionImage_botcap* pgvVisionImage = (gvVisionImage_botcap*)pgvVisionCCD->get_VisionImage();
...
{
    E_PROGRAM_STATUS eStatus = c_pgvVisionManager->gvMgr_getStatus();
    if( PSTATUS_Loaded == eStatus )
    {
        int iIndex = 0;
        ...
        {
            gvVisionCCD* pgvVisionCCD = c_pgvVisionManager->gvMgr_getVisionCCD(iIndex);
            if( nullptr == pgvVisionCCD )
                continue;
            gvVisionCam* pgvVisionCam = pgvVisionCCD->get_VisionCam();
            if( !pgvVisionCam->isInit() )
            {
                b1 = false;
                break;
            }
    ...
    {
        event.Enable( (PSTATUS_Inspecting == eStatus) ? true : false );
    }
}
...
{
    E_PROGRAM_STATUS eStatus = c_pgvVisionManager->gvMgr_getStatus();
    bool b1 = (PSTATUS_Loaded == eStatus) || ( PSTATUS_unLoad == eStatus );
    event.Enable(true);
}
...
void ISBOTCAPMain::OnToolDispUpdate( wxUpdateUIEvent& event )
{
    ...
    {
        event.Enable( (PSTATUS_Displaying == eStatus) ? true : false );
    }
}
...
void ISBOTCAPMain::OnToolSaveimgsClick( wxCommandEvent& event )
{
    ...
    {
        c_pgvVisionManager->gvMgr_runStart(PSTATUS_ImgSaving);
    }
}
...
void ISBOTCAPMain::OnToolSaveimgsUpdate( wxUpdateUIEvent& event )
{
    ...
    {
        event.Enable( (PSTATUS_ImgSaving == eStatus) ? true : false );
    }
}
...
void ISBOTCAPMain::OnToolCam1Update( wxUpdateUIEvent& event )
{
    int idxActiveCCD = 0;
    E_PROGRAM_STATUS eStatus = c_pgvVisionManager->gvMgr_getStatus();
    bool b1 = (PSTATUS_Inspecting != eStatus) && (PSTATUS_ImgSaving != eStatus);
    bool b2 = c_pgvVisionManager->gvMgr_getVisionCCD(idxActiveCCD)->get_using();
    event.Enable( b1&&b2 );
}
...
void ISBOTCAPMain::OnToolSaveimgClick( wxCommandEvent& event )
{
    wxFileDialog *filedlg = new wxFileDialog(this,
                                             ...
                                             wxGetCwd()+wxT("\\images"),
                                             wxEmptyString,
                                             wxT("JPG FILES(*.jpg)|*.jpg||"),
                                             wxFD_SAVE);
    if( wxID_OK == filedlg->ShowModal() )
    {
        int idxSelected = c_pgvVisionManager->get_idxActiveCCD();
        gvVisionCCD* pgvVisionCCD = c_pgvVisionManager->gvMgr_getVisionCCD(idxSelected);
        gvVisionImage* pgvVisionImage = pgvVisionCCD->get_VisionImage();
        pgvVisionImage->gvIMG_SaveImage(filedlg->GetPath());
    }
    filedlg->Destroy();
}
...
void ISBOTCAPMain::OnToolCamsetUpdate( wxUpdateUIEvent& event )
{
    ...
    bool b1 = c_pgvVisionManager->gvMgr_getStatus() == PSTATUS_Displaying;
    bool b2 = c_pgvVisionManager->gvMgr_getVisionCCD(idxSelected)->get_VisionCam()->isInit();
    event.Enable(b1&&b2);
}
...
{
    int idxSelected = c_pgvVisionManager->get_idxActiveCCD();
	gvVisionImage_botcap* pgvVisionImage = (gvVisionImage_botcap*)c_pgvVisionManager->gvMgr_getVisionCCD(idxSelected)->get_VisionImage();
